...
enum SimianFolderNaming {
    ...
    /// Multipart `filename=` for Simian: uses the local file’s name as-is. Date stamps (`_MMMdd.yy`) are not applied automatically; use **Add Date** on the Simian browser context menu when you want one.
    static func multipartUploadFilename(forLocalFileURL fileURL: URL) -> String {
